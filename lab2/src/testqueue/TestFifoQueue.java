...
public class TestFifoQueue {
	...
	public final void testPollOfEmpty() {
		assertEquals("Poll of empty queue should return null", null,
				myIntQueue.poll());
	}
	...
	public final void testIterator() {
		...
		Iterator<Integer> klasse = myIntQueue.iterator();
		boolean x = false;
		try {
			klasse.next();
		...
		assertTrue("WELL FUCK IT'S NOT ERROR", x);
		...
		Iterator<Integer> lasse = myIntQueue.iterator();

		assertEquals("WELLFUCK IT'S NOT 1", (int) lasse.next(), 1);
